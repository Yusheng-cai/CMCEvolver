...
namespace MeshTools
{
    ...
    Real CalculateBoundaryAverageHeight(Mesh& m, const std::vector<int>& BoundaryIndices);
    ...
    bool ShootingMethod_CA(Real& k, Mesh& m, AFP_shape* shape, MeshRefineStrategy* r, Real3 Volume_shift, Real init_k, Real init_step, Real max_k, Real goal_CA, \
                           Real tolerance=1e-3, bool overwrite_original_m=false, bool useNumerical=true);
